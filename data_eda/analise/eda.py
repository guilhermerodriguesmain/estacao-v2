...
class EDA:
    ...
    def valores_maximos_minimos(self, coluna_dados):
        if coluna_dados in self.df.columns:
            return self.df[coluna_dados].min(), self.df[coluna_dados].max()
        else:
            raise ValueError(f"A coluna '{coluna_dados}' não existe no dataframe.")
    ...
    def estatisticas_descritivas(self, coluna_dados):
        if coluna_dados in self.df.columns:
            return {
                "media": self.df[coluna_dados].mean(),
                "mediana": self.df[coluna_dados].median(),
                "desvio_padrao": self.df[coluna_dados].std()
            }
        else:
            raise ValueError(f"A coluna '{coluna_dados}' não existe no dataframe.")

        # Exemplo de retorno: {"media": 24.5, "mediana": 24.0, "desvio_padrao": 3.2}

    # Retorna possiveis anomalias ou outliers em cada variavel do dataframe usando o metodo do IQR (Interquartile Range)
    def detectar_anomalias_outliers(self, coluna_dados):
        if coluna_dados in self.df.columns:
            Q1 = self.df[coluna_dados].quantile(0.25)
            Q3 = self.df[coluna_dados].quantile(0.75)
            IQR = Q3 - Q1
            outliers = self.df[(self.df[coluna_dados] < (Q1 - 1.5 * IQR)) | (self.df[coluna_dados] > (Q3 + 1.5 * IQR))]
            return outliers
        else:
            raise ValueError(f"A coluna '{coluna_dados}' não existe no dataframe.")
    
    def salvar_dados(self, dados, caminho):
